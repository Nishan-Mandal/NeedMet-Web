...
export const buildListingPreviewFromFormData = ({
  formData,
  ...
  // convert local form images into preview images
  const previewImages = (formData.images || []).map((img, index) => {
    const previewUrl = img.preview || URL.createObjectURL(img.file);

    return new ImageFile({
      fileId: `preview-${index}`,
      fullUrl: previewUrl,
      thumbUrl: previewUrl,
    });
  });
